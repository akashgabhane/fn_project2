...
const handleError = (err) => {
    ...
    if (err.code === 11000) {
        ...
        Object.keys(errors).forEach(k => (!errors[k] && errors[k] !== undefined) && delete errors[k]); // check every key aur empty key delete krega
        return errors;
    }
    ...
    if (err.message.includes('college validation failed')) {
        ...
        Object.values(err.errors).forEach(({ properties }) => {
            errors[properties.path] = properties.message; //values and their error
        });
